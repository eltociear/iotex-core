...
func FromProtoBlobTxSideCar(pb *iotextypes.BlobTxSidecar) (*types.BlobTxSidecar, error) {
	...
	for i, v := range pb.Blobs {
		sidecar.Blobs[i] = kzg4844.Blob{}
		copy(sidecar.Blobs[i][:], v)
	}
	for i, v := range pb.Commitments {
		sidecar.Commitments[i] = kzg4844.Commitment{}
		copy(sidecar.Commitments[i][:], v)
	}
	for i, v := range pb.Proofs {
		sidecar.Proofs[i] = kzg4844.Proof{}
		copy(sidecar.Proofs[i][:], v)
	}
	return &sidecar, nil
}
